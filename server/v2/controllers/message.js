...
const Message = {
  ...
  async getAMailRecord(req, res) {
    ...
    const updateMailQuery = `UPDATE messageTable
  ...
  async deleteUserMail(req, res) {
    ...
    try {
      ...
      await db.query(senderDeletionUpdateQuery, senderValues);
      const receiverValues = [
        true,
        req.user.id,
        req.params.messageId,
      ];
      await db.query(receiverDeletionUpdateQuery, receiverValues);
      const deleteValues = [
        true,
        req.params.messageId,
      ];
