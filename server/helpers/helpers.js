...
const Helper = {
  ...
  validateForgetPassword(data) {
    if (!ValidData.isLength(data.newPassword, { min: 8, max: 30 })) {
      errors.newPassword = 'Password must be between 8 and 30 characters';
    }
    if (ValidData.isEmpty(data.newPassword)) { errors.newPassword = 'The Password field is required'; }
    if (isValueEmpty(data.confirmNewPassword)) { errors.confirmNewPassword = 'Confirm password field is required'; }
    if (!ValidData.equals(data.newPassword, data.confirmNewPassword)) { errors.confirmNewPassword = 'Your password does not match'; }
