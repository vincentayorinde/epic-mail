...
var Helper = {
  ...
  validateForgetPassword: function validateForgetPassword(data) {
    if (!_validator["default"].isLength(data.newPassword, {
      min: 8,
      max: 30
    })) {
    ...
    if (_validator["default"].isEmpty(data.newPassword)) {
      errors.newPassword = 'The Password field is required';
    }

    if ((0, _isValueEmply["default"])(data.confirmNewPassword)) {
      errors.confirmNewPassword = 'Confirm password field is required';
    }

    if (!_validator["default"].equals(data.newPassword, data.confirmNewPassword)) {
      errors.confirmNewPassword = 'Your password does not match';
    }
