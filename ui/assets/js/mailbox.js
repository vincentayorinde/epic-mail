...
fetch('http://localhost:5000/api/v2/messages', {
  ...
  .then((data) => {
    ...
    if (data) {
      ...
      data.data.rows.forEach((message) => {
        ...
        if (message.status === 'unread') {
          theOutput += `
          <div onclick="showMessageContent(${message.id});" id="${message.id}" class="record">
          <span>${message.createon}</span> <i onclick="deleteMessage(${message.id})" class="fas fa-trash-alt del"></i><br>
          <span> <h3><i class="fas fa-circle"></i> From ${message.senderid}</h3> </span>
          ...
          <div onclick="showMessageContent(${message.id});" id="${message.id}" class="record">
          <span>${message.createon}</span> <i onclick="deleteMessage(${message.id})" class="fas fa-trash-alt del"></i><br>
          <span> <h3>From ${message.senderid}</h3> </span>
          <h4>${message.subject}</h4>
          <p>${message.message}</p>
        </div>`;
        }
    ...
    document.getElementById('box-5-data').innerHTML = theOutput;
  })
  .catch((err) => {
    console.log(err);
  });
